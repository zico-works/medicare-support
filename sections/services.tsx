...
const Services = () => {
  ...
  const [screen, setScreen] = React.useState('-76.4%');
  ...
  return (
    <section id='services'>
      <div ref={container} className='relative md:h-[300vh]'>
        <div className='sticky left-0 top-5 flex items-center overflow-hidden md:h-screen'>
          <motion.div
          ...
          >
            {SERVICES.map(({ color, desc, src, title }, idx) => (
              <div
                key={title}
                className={cn(
                  'flex flex-col justify-center rounded-xl px-4  transition duration-200 h-[85vh] hover:bg-[#EFF7FF]',
                  idx === 0 ? 'md:w-[250px]' : 'md:w-[600px]',
                )}
              ...
              >
                ...
                {idx === 0 ? (
                  <div
                    ...
                    className='text-2xl font-semibold text-primary-600 sm:text-3xl lg:text-4xl'
                  />
                ) : (
